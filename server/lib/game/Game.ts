...
export class Game {
  ...
  chancellorProposedVeto: boolean = false;

  presidentDrewCards: boolean = false;
  ...
  reconnectPlayer(permaId: string, socketId: string): Player | null {
  ...
  startGame(playersOrder?: string[]) {
    if (this.started) return;
    this.players = this.players.filter((p) => (p.connected = true));
    this.started = true;
  ...
  startNewRound(): void {
    ...
    this.presidentDrewCards = false;
    this.presidentExecutedPlayer = false;
    this.presidentInvestigatedPlayer = false;
    this.presidentInvestigation = null;

    // Double check deck size - usually checked after enactment, but needed here in case of veto
    this.replenishFromDiscardIfNeeded();
  }
  ...
  handleInvestigateModalClose(playerIndex: number): void {
    ...
    this.presidentInvestigatedPlayer = true;
    this.players[playerIndex].investigated = true;
  ...
  handleExecutionModalClose(playerIndex: number): void {
    ...
    this.presidentExecutedPlayer = true;
