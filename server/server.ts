...
io.on("connection", (socket: Socket) => {
  ...
  socket.on(
    ...
    (
      ...
      if (roomManager.exists(roomName)) {
        return callback({
          error: "Game name already exists. Pick another name",
        });
      }
      ...
      callback({ roomId: room.id, players: room.players });
  ...
  socket.on(
    ...
    (
      ...
      if (room.isFull) return callback({ error: "Room full." });

      if (room.players.some((p) => p.name === name)) {
        return callback({ error: "Name exists in the room already." });
      }
  ...
  socket.on("start_manual_game", ({ roomId, playersId }) => {
    ...
    const game = room.startGame(playersId);
    io.in(room.id).emit("game_start", {
      roomId: room.id,
      players: game.players,
      hostId: room.hostId,
  ...
  socket.on(
    ...
    ({ roomId, permaId }: { roomId: string; permaId: string }, callback) => {
      ...
      const player = room.reconnectPlayer(permaId, socket.id);
      if (!player) {
