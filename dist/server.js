...
io.on("connection", (socket) => {
    ...
    socket.on("check_room", ({ roomId }, cb) => {
        const exists = !!rooms[roomId] || // if you're tracking rooms yourself
            io.sockets.adapter.rooms.has(roomId); // adapter-level check
        if (typeof cb === "function")
            cb({ exists });
        else
            socket.emit("room_check_result", { exists });
    });
    // CREATE GAME
    ...
    socket.on("join_game", (data, callback) => {
        ...
        if (room.players.length === room.maxPlayers) {
            ...
            room.game = new Game(roomId, room.hostId, room.players.map((p) => ({
                id: p.id,
                permaId: p.permaId,
                name: p.name,
                portrait: p.portrait,
    ...
    socket.on("disconnect", () => {
        ...
        setTimeout(() => {
            const stillDisconnected = room.players.find((p) => p.id === player.id)?.id === null;
    ...
    socket.on("reconnect_to_room", ({ roomId, permaId }, callback) => {
    ...
    socket.on("setStatusBanner", ({ roomId, playerId, statusBanner, single = true }) => {
        const room = rooms[roomId];
        if (!room || !room.game)
            return;
    ...
    // SELECT CHANCELLOR
    socket.on("handleNominateChancellorModalClose", ({ roomId, chancellorId }) => {
        const room = rooms[roomId];
        if (!room || !room.game)
            return;
    ...
    socket.on("handleVoteModalClose", ({ roomId, playerIndex, playerId, vote }) => {
        const room = rooms[roomId];
        if (!room || !room.game)
            return;
    ...
    socket.on("handlePresidentHandModalClose", ({ roomId, playerIndex, discard }) => {
        const room = rooms[roomId];
        if (!room || !room.game)
            return;
    ...
    socket.on("handleElectionTrackerModalClose", ({ roomId, playerIndex, playerId }) => {
        const room = rooms[roomId];
        if (!room || !room.game)
            return;
    ...
    socket.on("handlePolicyEnactedModalClose", ({ roomId, playerIndex, playerId }) => {
        const room = rooms[roomId];
        if (!room || !room.game)
            return;
    ...
    socket.on("handleSpecialElectionModalClose", ({ roomId, newPresidentIndex }) => {
        const room = rooms[roomId];
        if (!room || !room.game)
            return;
    ...
    socket.on("handleProposeVetoModalClose", ({ roomId, playerIndex, oblige }) => {
        const room = rooms[roomId];
        if (!room || !room.game)
            return;
    ...
    socket.on("gameEnded", ({ roomId, playerId, action }) => {
        ...
        if (!room || !room.game)
            return;
        if (action === "restart") {
            room.game = new Game(roomId, room.hostId, room.game.players.map((p) => ({
                id: p.id,
                permaId: p.permaId,
                name: p.name,
                portrait: p.portrait,
...
httpServer.listen(process.env.SOCKET_PORT, () => console.log(`Socket.io server running on port ${process.env.SOCKET_PORT}`));
