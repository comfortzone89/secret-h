...
export class Game {
    ...
    drawPile = [];
    discardPile = [];
    enactedPolicies = []; // historical order
    /* Government & term limits */
    currentPresidentIndex = null;
    ...
    previousPresidentialOrder = null;
    failedToElectGovernment = false;
    gameWon = null;
    winReason = null;
    playerLeft = false;
    ...
    constructor(id, hostId, lobbyPlayers) {
        this.id = id;
        this.hostId = hostId;
    ...
    assignRoles() {
        ...
        if (total <= 6)
            ...
            totalFascists = 4; // 9–10 players: 3 fascists + Hitler
    ...
    setStatusBanner(playerIndex, text, loading = false) {
        this.players[playerIndex].statusBanner = { text, loading };
    }
    setStatusBannerAll(text, loading = false) {
        this.players.map((p) => (p.statusBanner = { text, loading }));
    }
    setVote(playerIndex, vote) {
    ...
    setPresidentHand(cards = []) {
        this.presidentHand = cards;
    }
    setChancellorHand(cards = []) {
        this.chancellorHand = cards;
    }
    setPresidentDiscard(discard) {
        const discardedPolicy = this.presidentHand[Number(discard)];
        const remainingCards = this.presidentHand.filter((_, i) => i !== Number(discard));
    ...
    drawAndEnactPolicy() {
        ...
        this.setStatusBannerAll(`Policy enacted: ${policyName.charAt(0).toUpperCase() + policyName.slice(1)}`);
    }
    setChancellorEnact(policyIndex) {
        const policy = this.chancellorHand[policyIndex];
        const discardedPolicy = this.chancellorHand.splice(policyIndex, 1)[0];
        this.setChancellorHand([]);
        this.discardPolicy(discardedPolicy);
        this.enactPolicy(policy);
    }
    setModalConfirm(playerIndex) {
    ...
    setLastGovernment(presidentId, chancellorId) {
        this.lastGovernment = { presidentId, chancellorId };
    }
    setPresident(presidentIndex) {
        this.currentPresidentIndex = presidentIndex;
    }
    startNewRound() {
        if (this.electionTracker < 3) {
            if (!this.failedToElectGovernment) {
                this.setLastGovernment(this.currentPresidentIndex, this.currentChancellorIndex);
            }
        }
        else {
    ...
    /* HANDLERS */
    handleRoleModalClose(playerIndex) {
        ...
        if (allConfirmed) {
            ...
            this.setStatusBanner(playerIndex, "Waiting for other players to start the game...", true);
        }
        return allConfirmed;
    }
    ...
    handleVoteModalClose(playerIndex, vote) {
        ...
        if (allVoted) {
            ...
            this.showVotes = true;
            this.setStatusBannerAll(`${yesVotes} - ${noVotes}: Vote ${yesVotes > noVotes ? "passed" : "failed"}`);
            if (yesVotes > noVotes) {
    ...
    handlePresidentHandModalClose(playerIndex, discard) {
        ...
        this.setStatusBannerAll("Waiting for Chancellor to enact a policy...", true);
    }
    ...
    handlePolicyEnactedModalClose(playerIndex) {
        this.setModal(playerIndex, null);
        ...
        if (presidentialAction &&
            ...
            if (this.currentPresidentIndex === playerIndex) {
                ...
                this.setStatusBanner(playerIndex, "Waiting for President to make use of his Presidential power...", true);
            }
        }
        else {
            if (this.currentPresidentIndex === playerIndex) {
                ...
                this.setStatusBanner(playerIndex, "Waiting for President to end his term...", true);
            }
        }
    }
    handleElectionTrackerModalClose(playerIndex) {
        if (this.electionTracker >= 3) {
            if (playerIndex === this.currentPresidentIndex) {
                ...
                this.setStatusBanner(playerIndex, "Waiting for President to draw a card from the top of the deck and play it...", true);
                this.setModal(playerIndex, null);
            }
        }
        else {
    ...
    handlePeekModalClose(playerIndex) {
        this.setModal(playerIndex, null);
    ...
    handleInvestigateResultModalClose(playerIndex) {
        if (this.currentPresidentIndex === playerIndex) {
    ...
    handleExecutionResultModalClose(playerIndex) {
    ...
    handleVetoUnlockedModalClose(playerIndex) {
        this.setModal(playerIndex, null);
        if (this.currentPresidentIndex === playerIndex) {
            this.players[playerIndex].endTerm = true;
            this.setStatusBanner(playerIndex, "End your term to continue");
    ...
    handleShowPresidentHand(playerIndex) {
        if (playerIndex === this.currentPresidentIndex) {
            ...
            this.setStatusBanner(playerIndex, "Waiting for President to select a policy to discard...", true);
        }
        this.resetVotes();
    }
    handleVeto(playerIndex) {
        this.chancellorProposedVeto = true;
        this.setModal(playerIndex, null);
        const presidentIndex = this.currentPresidentIndex;
        this.setModal(presidentIndex, "vetoProposed");
        this.setStatusBannerAll("Chancellor proposed a veto! Waiting for President to take action...", true);
    }
    handleProposeVetoModalClose(playerIndex, oblige) {
        if (oblige) {
            this.voteFailed();
        }
        else {
    ...
    /* ---------------- Win checks ---------------- */
    checkWin(justPassedPolicy = false) {
