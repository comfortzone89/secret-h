...
const GameOverModal: React.FC = () => {
  ...
  const fascists = gameInstance?.players.filter((p) => p.party === "fascist");
  const liberals = gameInstance?.players.filter((p) => p.party === "liberal");

  return (
    <Modal modal="gameOver" className="bg-black">
      <div className="mb-4">
        <Image
        ...
        <Button
          onClick={() =>
            handleGameOverModalClose(
              gameInstance?.hostId === me?.id && !gameInstance?.playerLeft
                ? "restart"
                : "lobby"
            )
          }
